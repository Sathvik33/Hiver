    ...
    async def classify_intent(self, message: str, context: Optional[str] = None) -> IntentPrediction:
        ...
        try:
            ...
            return IntentPrediction(
                ...
                reason=parsed.get("reason", "Classified via Groq API")
            )
        except Exception as e:
            logger.warning(f"Groq intent classification error: {e}")
    ...
    async def generate_grounded_reply(
        ...
        try:
            ...
            return GeneratedReply(
                reply=parsed.get("reply", "Please DM us your order details."),
                confidence=float(parsed.get("confidence", 0.85)),
                evidence_ids=parsed.get("evidence_ids", []),
                supported_claims=parsed.get("supported_claims", [])
            )
        except Exception as e:
            logger.warning(f"Groq reply generation error: {e}")
            return GeneratedReply(
    ...
    async def judge_reply(
        ...
        try:
            ...
            return LLMJudgeEvaluation(
                ...
                style=sty, safety=saf, overall=round(overall, 2), reason=p.get("reason", "Graded by Groq LLM")
            )
        except Exception as e:
            return LLMJudgeEvaluation(
                correctness=3, relevance=3, grounding=3, completeness=3, style=4, safety=4,
                overall=3.33, reason=f"Groq evaluation error: {e}"
            )
